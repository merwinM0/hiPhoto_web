...
use config::Config;
use handlers::{auth, user, room, photo, score};
...
fn create_app(pool: SqlitePool, config: Config) -> Router {
    ...
    let protected_routes = Router::new()
        ...
        .route("/api/tags/:tag_id", delete(handlers::tag::delete_tag))
        .route("/api/photos/:photo_id/tags", get(handlers::tag::get_photo_tags))
        // 评分相关
        ...
        .layer(middleware::from_fn_with_state(config.clone(), middleware::auth::auth_middleware));
