...
impl EmailService {
    ...
    pub fn new(config: Config) -> Self {
        ...
        let transport = match config.email_transport.as_str() {
            ...
            _ => {
                // 默认使用控制台模式
                tracing::warn!("Unknown email transport: {}, using console mode", config.email_transport);
                EmailTransport::Console
            }
        };
    ...
    async fn send_via_smtp(&self, message: &Message) -> Result<()> {
        tracing::debug!("Connecting to SMTP server: {}:{}", self.config.smtp_host, self.config.smtp_port);
        ...
        let mailer = mailer_builder
            ...
            .timeout(Some(std::time::Duration::from_secs(self.config.smtp_timeout)))
            .build();
        
        // 添加重试机制
        let mut last_error = None;
        for attempt in 1..=self.config.smtp_max_retries {
            tracing::debug!("SMTP send attempt {}/{}", attempt, self.config.smtp_max_retries);
            ...
            match mailer.send(message) {
                ...
                Err(e) => {
                    ...
                    if attempt < self.config.smtp_max_retries {
                        ...
                        tracing::warn!("SMTP send failed (attempt {}), retrying in {:?}: {}", attempt, delay, last_error.as_ref().unwrap());
